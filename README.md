...
# SDK для интеграции с программным комплексом http://fivepost.ru.

> [!IMPORTANT]
> **Неофициальный независимый проект**
>
> Этот проект не является официальным SDK 5post, не связан с ООО «Файв Пост»,
> не поддерживается, не одобрен и не спонсируется правообладателем.
>
> Обозначение «5post» используется исключительно для указания совместимости
> SDK с API соответствующего сервиса. Все права на товарный знак «5post»
> принадлежат его правообладателю.
>
> **Unofficial independent project**
>
> This is not an official 5post SDK. The project is not affiliated with,
> maintained, endorsed, or sponsored by ООО “Файв Пост” or the trademark owner.
> The “5post” name is used solely to identify API compatibility.
...
try {
    ...
    // $e->getCode(); http код ответа сервиса
    // $e->getRawResponse(); // ответ сервера  как есть (http request body)
}
...
<?php
    try {
        ...
        for ($i = 0; $i < $totalPages; $i++) {
            $result = $Client->getPvzList($i, 1000); // Больше 2000 за раз получить нельзя
                    ...
                    (
                        [0] => Array
                            (
                                ...
                                [additional] => Постамат  расположен в магазине «Пятёрочка»
                                [cellLimits] => Array
                                    (
                                        [maxCellWidth] => 457
                                        [maxCellHeight] => 322
        ...
        **/
        ...
        // Обработка ошибки вызова API
        // $e->getMessage(); // текст ошибки
        // $e->getCode(); // http код ответа сервиса  или код ошибки при наличии узла error в ответе
        // $e->getRawResponse(); // ответ сервера  как есть (http request body)
    }
...
Заказ можно запросить по своему уникальному номеру или по номеру в системе .
...
- *string|null $vendor_id* - ID заказа в системе .
...
Метод **getReceiptsByListVendorIds** возвращает данные чеков по списку заказов в системе .

**Входные параметры:**
- *string[] $order_ids* - Список id заказов клиента или ;
...
<?php
    try {
        ...
        // По списку ID заказов в системе
